...
public class BotClientLoginNetHandler implements IClientLoginNetHandler {
   public static final Logger LOGGER = LogManager.getLogger();
   public final Minecraft mc;
   @Nullable
   public final Screen previousGuiScreen;
   public final Consumer<ITextComponent> statusMessageConsumer;
   public final BotNetwork networkManager;
   public final String requestedName;
   public GameProfile gameProfile;

   public BotClientLoginNetHandler(BotNetwork networkManagerIn, Minecraft mcIn, @Nullable Screen previousScreen, Consumer<ITextComponent> statusMessageConsumerIn, String requestedName) {
   ...
   private ITextComponent joinServer(String serverHash) {
      try {
         ...
         return new TranslationTextComponent("disconnect.loginFailedInfo", new Object[]{new TranslationTextComponent("disconnect.loginFailedInfo.insufficientPrivileges")});
      } catch (AuthenticationException var6) {
         return new TranslationTextComponent("disconnect.loginFailedInfo", new Object[]{var6.getMessage()});
      }
   }
   ...
   public void handleLoginSuccess(SLoginSuccessPacket packetIn) {
      ...
      LOGGER.info("Bot login success [{} -> {}]", this.requestedName, this.gameProfile.getName());
      this.networkManager.setConnectionState(ProtocolType.PLAY);
      this.networkManager.setNetHandler(new BotClientPlayNetHandler(this.mc, this.previousGuiScreen, this.networkManager, this.gameProfile));
   }

   public void onDisconnect(ITextComponent reason) {
      String botName = this.gameProfile != null ? this.gameProfile.getName() : this.requestedName;
      bots.BotManager.pendingNetworks.remove(this.networkManager);
      LOGGER.warn("Bot login disconnected [{}]: {}", botName, reason.getString());
      if (this.mc.ingameGUI != null) {
         this.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion((new StringTextComponent("Бота отключило на логине: ")).append(reason), 0);
      } else if (this.previousGuiScreen != null && this.previousGuiScreen instanceof RealmsScreen) {
