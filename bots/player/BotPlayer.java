...
import tech.neyrox.event.impl.EventChangeSprint;
import tech.neyrox.event.impl.EventChat;
import tech.neyrox.event.impl.EventInvClose;
import tech.neyrox.event.impl.EventNoSlow;
import tech.neyrox.modules.impl.movement.AutoSprint;
import tech.neyrox.modules.impl.player.NoPush;
import tech.neyrox.utils.other.StopWatch;
...
public class BotPlayer extends AbstractClientPlayerEntity {
   ...
   public void closeScreen() {
      EventInvClose event = new EventInvClose();
      event.call();
      if (!event.isCancelled()) {
   ...
   private boolean shouldBlockPushPlayer(BlockPos pos) {
      if (((NoPush)Neyrox.getInstance().moduleManager.find(NoPush.class)).isEnabled() && ((NoPush)Neyrox.getInstance().moduleManager.find(NoPush.class)).elements.isEnabled("Блоки")) {
         return false;
      } else {
   ...
   public void livingTick() {
      ...
      boolean flag4 = ((AutoSprint)Neyrox.getInstance().moduleManager.find(AutoSprint.class)).isEnabled() && ((AutoSprint)Neyrox.getInstance().moduleManager.find(AutoSprint.class)).ignoreFood.getValue() || (float)this.getFoodStats().getFoodLevel() > 6.0F || this.abilities.allowFlying;
      if ((this.onGround || this.canSwim()) && !flag1 && !flag2 && this.isUsingSwimmingAnimation() && !this.isSprinting() && flag4 && !this.isHandActive() && !this.isPotionActive(Effects.BLINDNESS)) {
         label250: {
            label249: {
               if (this.sprintToggleTimer <= 0) {
                  if (((AutoSprint)Neyrox.getInstance().moduleManager.find(AutoSprint.class)).isEnabled()) {
      ...
      if (!this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && !this.isHandActive() && !this.isPotionActive(Effects.BLINDNESS)) {
         EventChangeSprint eventChangeSprint = (EventChangeSprint)(new EventChangeSprint(((AutoSprint)Neyrox.getInstance().moduleManager.find(AutoSprint.class)).isEnabled() ? this.movementInput.isMovingForward() : this.mc.gameSettings.keyBindSprint.isKeyDown())).call();
